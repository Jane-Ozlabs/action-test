      ...
      <div class="tableBox">
        <table>
          <tr>
            ...
            <th class="radiusLeft">
              <p>Bettingspoon</p>
              <span class="absText2">Revenue share</span>
            </th>
          ...
          <tr v-for="x of rows" :key="x.id">
            ...
            <td :color="x.winLose < 0 ? '#dd3100' : '#006cff'">
              {{ x.winLose | formatCurrency }}
            </td>
            <td>{{ x.rS0 | formatCurrency }}</td>
            <td>{{ x.rS1 | formatCurrency }}</td>
            <td :color="x.status == 'pending' ? '#dd3100' : '#000'">
              {{ x.status }}
            </td>
          </tr>
        </table>
  ...
  methods: {
    ...
    async updateFilters() {
      if (this.$refs.header) {
        ...
        this.filters.dateTo = moment(
          this.$refs.header.filters.dateTo,
          'YYYY-MM',
        )
          .endOf('month')
          .add(1, 'day')
