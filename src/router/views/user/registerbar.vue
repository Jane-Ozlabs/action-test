...
export default {
    ...
    computed: {
        ...
        selectedAgentGroup() {
            return this.agentGroups.find((x) => x.id == this.group) || { r1: 0, r2: 0, r3: 0 };
        },
        ...mapState({
            registrationModalResult: (state) => {
                return (state.modals["account-registration-modal"] && !state.modals["account-registration-modal"].isVisible && state.modals["account-registration-modal"].result) || null;
            },
        }),
    },
    data() {
