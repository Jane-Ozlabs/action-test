<template>
  <div id="agent-filter-user" class="selectBox">
    ...
    <BSSelect ref="id2" v-model="filters.id2" :options="a2" :readonly="filters.id1 == 0 || filters.id1 == '+' || agentLevel >= 2" />
    <BSSelect ref="id3" v-model="filters.id3" :options="a3" :readonly="filters.id2 == 0 || filters.id2 == '+' || agentLevel >= 3" />
...
export default {
  components: { BSSelect },
  props: [ "new", "agentLines", "noTitle", "noSearch", "onAction", "action" ],
  data() {
    return {
  ...
  computed: {
    agentLevel() { return GetAuth().userAgent.agentLevel; },
  ...
  methods: {
    async update() {
      ...
      !a1.find(x => x.id == this.filters.id1) && this.filters.id1 != '+' && (this.filters.id1 = 0)
      !a2.find(x => x.id == this.filters.id2) && this.filters.id2 != '+' && (this.filters.id2 = 0)
      !a3.find(x => x.id == this.filters.id3) && (this.filters.id2 == '+' || this.filters.id3 != '+') && (this.filters.id3 = 0)
      ...
      this.a1 = a1.map(x => ({ text: x.name, value: x.id })); this.new && this.a1.unshift({ text: "New", value: "+" }); this.a1.unshift({ text: "Level 1", value: 0 }); 
      this.a2 = a2.map(x => ({ text: x.name, value: x.id })); this.new && this.a2.unshift({ text: "New", value: "+" }); this.a2.unshift({ text: "Level 2", value: 0 });
      if(this.new && this.agentLevel < 3) this.a3 = []; else this.a3 = a3.map(x => ({ text: x.name, value: x.id })); this.new && this.a3.unshift({ text: "New", value: "+" }); this.a3.unshift({ text: "Level 3", value: 0 });
