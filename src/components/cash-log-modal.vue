<template>
  <div :class="['detail_popup']" :style="styleObject" :id="MODAL_KEY">
    ...
    <div class="contents">
      <div class="summary">
        Period : {{ summary.dateFrom | formatDate }} ~
        {{ summary.dateTo | formatDate }}
        <b>Cash balance($) : {{ summary.total | formatCurrency }}</b>
      </div>
      <div class="scrollBox">
        <div class="tableBox">
          <ul class="thBox">
          ...
          <ul class="tdBox">
            <li
              v-for="item of rows"
              :key="item.id"
              :class="item.transaction == 'withdrawal' ? 'withdrawal' : ''"
            >
              <p class="item01-2">
...
export default {
  ...
  computed: {
    ...mapState({
      visible: state =>
        state.modals[MODAL_KEY] && state.modals[MODAL_KEY].isVisible,
      title: state =>
        (state.modals[MODAL_KEY] && state.modals[MODAL_KEY].title) ||
        'Deposit/Withdrawal',
      initialFilters: state =>
        state.modals[MODAL_KEY] && state.modals[MODAL_KEY].filters,
      userId: state =>
        state.modals[MODAL_KEY] && state.modals[MODAL_KEY].userId,
    }),
    styleObject() {
